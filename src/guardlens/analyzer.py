...
to Ollama for vision analysis. Everything else consumes typed
:class:`ScreenAnalysis` objects.
...
def _extract_classification(tool_calls: list[dict[str, Any]]) -> ThreatClassification:
    ...
    """
    args = find_call(tool_calls, "classify_threat")
    if args is None:
...
def _extract_grooming_stage(tool_calls: list[dict[str, Any]]) -> GroomingStageResult | None:
    args = find_call(tool_calls, "identify_grooming_stage")
    if args is None:
        return None
...
def _extract_parent_alert(tool_calls: list[dict[str, Any]]) -> ParentAlert | None:
    args = find_call(tool_calls, "generate_parent_alert")
    if args is None:
        return None
